...
namespace CPWizard
{
    public class DirectInputManager : IDisposable
    {
        ...
        private Joystick[] m_joyDevice = null;
        ...
        public DirectInputManager(Control control, object tag)
        {
            m_control = control;
            m_tag = tag;
        }
        ...
        public void ResourceLoad()
        {
            ...
            var gameControllerList = m_directInput.GetDevices(DeviceType.Gamepad, DeviceEnumerationFlags.AllDevices);

            m_joyCount = 0;
            m_joyArray = new JoyNode[gameControllerList.Count];
            m_joyDevice = new Joystick[gameControllerList.Count];
            ...
            {
                var joystick = new Joystick(m_directInput, deviceInstance.InstanceGuid);
                joystick.SetCooperativeLevel(m_control.Handle, CooperativeLevel.NonExclusive | CooperativeLevel.Background);
                joystick.Properties.AxisMode = DeviceAxisMode.Absolute;
                SetAxisRange(joystick, AXIS_MIN, AXIS_MAX);
                joystick.Acquire();
                m_joyDevice[m_joyCount++] = joystick;
            }
        ...
        private void SetAxisRange(Joystick joystick, int lower, int upper)
        {
            foreach (var deviceObject in joystick.GetObjects())
            {
                if ((deviceObject.ObjectId.Flags & DeviceObjectTypeFlags.Axis) != 0)
                    joystick.GetObjectPropertiesById(deviceObject.ObjectId).Range = new InputRange(lower, upper);
            }
        }
        ...
        public bool TryPollJoystick(float elapsedTime)
        {
            ...
            try
            {
                ...
                {
                    var joystick = m_joyDevice[i];
                    if (joystick == null) continue;
                    var joy = m_joyArray[i];
                    ...
                    joy.Pov = (ushort)joyState.PointOfViewControllers[0];
        ...
        public static string[] GetJoystickArray()
        {
            ...
            {
                foreach (var instance in directInput.GetDevices(DeviceType.Gamepad, DeviceEnumerationFlags.AttachedOnly))
